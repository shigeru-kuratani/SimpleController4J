...
/**
 * <p>ジョインポント(Before) アノテーション</p>
 * <pre>
 * このアノテーションを記述するとexcution値がディスパッチメソッド名にマッチする場合は、
 * ディスパッチメソッド実行前に、このアノテーションを記述したメソッドが実行される。
 * (ex.)[at]Before(execution = "com.somecompany.service.UserService.init")
 *        → ディスパッチメソッドの完全修飾名が「com.somecompany.service.UserService.init」の場合に実行されます。
 * (ex.)[at]Before(execution = "*.UserService.init")
 *        → ディスパッチメソッドが「クラス名：UserService」「メソッド名：init」の場合に全てのパッケージで実行されます。
 * (ex.)[at]Before(execution = "*.init")
